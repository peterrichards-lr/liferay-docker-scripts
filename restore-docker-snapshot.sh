...
while [[ $# -gt 0 ]]; do
	...
	-i | --index)
		shift
		[[ -z "$1" || ! "$1" =~ '^[0-9]+$' ]] && _die "--index requires a numeric value"
		INDEX_ARG="$1"
		;;
	--checkpoint)
		shift
		[[ -z "$1" ]] && _die "--checkpoint requires a folder name"
		CHECKPOINT_ARG="$1"
		;;
	--no-list)
		NO_LIST=1
	...
	--format)
		...
		case "$1" in standard | liferay-cloud) FORMAT_OVERRIDE="$1" ;; *) _die "--format must be standard|liferay-cloud" ;; esac
		;;
	--min-meta-version)
		shift
		[[ -z "$1" || "$1" != <-> ]] && _die "--min-meta-version requires an integer"
		MIN_META_VERSION="$1"
		;;
	--allow-legacy)
		ALLOW_LEGACY=1
...
if [[ "$snapshot_type" == "hypersonic" ]]; then
	...
	if echo "$snapshot_type" | grep -qi "postgresql"; then
		...
		if [[ "$dump_file" == *.gz ]]; then
			gunzip -c "$dump_file" | mysql -h "$myhost" -P "$myport" -u "$jdbc_user" -p"$jdbc_pass" "$dbname" >/dev/null
