...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo "  -t, --tag <tag>               Liferay Docker image tag. Default: latest LTS detected";
...
LIFERAY_TAG_DEFAULT=$(curl -s 'https://hub.docker.com/v2/repositories/liferay/dxp/tags?page_size=2048&name=-lts&ordering=name' | jq -r --arg year "$(date +%Y)" '.results[].name | select(startswith($year)) | select(test("^[0-9]{4}\\.q[0-9]+\\.[0-9]+-lts$"))' | sort -V | tail -n1)
if [[ -z "$LIFERAY_TAG_DEFAULT" ]]; then
  LIFERAY_TAG_DEFAULT=$(curl -s 'https://hub.docker.com/v2/repositories/liferay/dxp/tags?page_size=2048&name=-lts&ordering=name' | jq -r '.results[].name | select(test("^[0-9]{4}\\.q[0-9]+\\.[0-9]+-lts$"))' | sort -V | tail -n1)
fi
if [[ -z "$LIFERAY_TAG_DEFAULT" ]]; then info_custom "${Yellow}Could not auto-detect an -lts Docker tag. Please enter one manually."; fi
...
if [[ -n "$ROOT_ARG" ]]; then
  ...
  if [[ $NON_INTERACTIVE -eq 1 ]]; then LIFERAY_ROOT="$LIFERAY_ROOT_DEFAULT"; else read_config "Liferay Root" LIFERAY_ROOT "$LIFERAY_ROOT_DEFAULT"; fi
fi

CONTAINER_NAME=${CONTAINER_ARG:-$(echo "$LIFERAY_ROOT" | sed -e 's:.*/::' -e 's/[\.]/-/g')}
IMAGE_TAG=$IMAGE_NAME:$LIFERAY_TAG
...
if [ $? -eq 1 ]; then
  ...
  if [[ -n "$DB_KIND" ]]; then
    if [[ "$DB_KIND" == "hypersonic" ]]; then USE_HYPERSONIC=Y; else USE_HYPERSONIC=N; LIFERAY_DATABASE="$DB_KIND"; fi
  else
    if [[ $NON_INTERACTIVE -eq 1 ]]; then USE_HYPERSONIC=Y; else read_config "Use Hypersonic database" USE_HYPERSONIC N; fi
  fi
  if [[ "${USE_HYPERSONIC:u}" == "N" ]]; then
    ...
    DATABASE_NAME=$(echo "$CONTAINER_NAME" | tr -d '-')
  ...
  if [[ "${REMOVE_STATE_FOLDER:u}" == "Y" ]]; then
    ...
    rm -R "$OSGI_STATE_VOLUME" 2>/dev/null
    mkdir -p "$OSGI_STATE_VOLUME"
  fi
