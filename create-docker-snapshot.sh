...
while [[ $# -gt 0 ]]; do
	...
	-b | --backups-dir)
		shift
		[[ -z "$1" ]] && _die "--backups-dir requires a path"
	...
	--pg-host)
		shift
		[[ -z "$1" ]] && _die "--pg-host requires a value"
		PG_HOST_OVERRIDE="$1"
		;;
	--pg-port)
		shift
		[[ -z "$1" ]] && _die "--pg-port requires a value"
		PG_PORT_OVERRIDE="$1"
		;;
	--my-host)
		shift
		[[ -z "$1" ]] && _die "--my-host requires a value"
		MY_HOST_OVERRIDE="$1"
		;;
	--my-port)
		shift
		[[ -z "$1" ]] && _die "--my-port requires a value"
		MY_PORT_OVERRIDE="$1"
		;;
	--tag)
		shift
		[[ -z "$1" || "$1" != *"="* ]] && _die "--tag requires key=value"
		TAGS+="$1"
		;;
	--retention)
		shift
		[[ -z "$1" || ! "$1" =~ '^[0-9]+$' ]] && _die "--retention requires an integer"
		RETENTION_N="$1"
		;;
	--format)
		shift
		[[ -z "$1" ]] && _die "--format requires standard|liferay-cloud"
		case "$1" in
	...
	-n | --name)
		shift
		[[ -z "$1" ]] && _die "--name requires a value"
		SNAPSHOT_NAME_ARG="$1"
		;;
	--no-name)
		NO_SNAPSHOT_NAME=1
...
if [[ $FILES_ONLY -eq 0 ]]; then
	if [[ "$snap_type" == "postgresql" ]]; then
		...
		if [[ "$BACKUP_FORMAT" == "liferay-cloud" ]]; then
			...
			dump_cmd="pg_dump -h $pghost -p $pgport -U $jdbc_user -d $dbname -F p --no-owner --no-privileges | gzip -c"
		else
			dump_file="$checkpoint_dir/db-postgresql.sql$COMPRESS_EXT"
			dump_cmd="pg_dump -h $pghost -p $pgport -U $jdbc_user -d $dbname | eval $COMPRESS_CMD"
		fi
		info "Dumping PostgreSQL database: $dbname"
		PGPASSWORD="$jdbc_pass" eval "$dump_cmd" >"$dump_file"
		if [[ $? -ne 0 || ! -s "$dump_file" ]]; then
			rm -f "$dump_file"
			_die "Database dump failed (PostgreSQL)."
		fi
		...
		if [[ "$BACKUP_FORMAT" == "liferay-cloud" ]]; then
			dump_file="$checkpoint_dir/database.gz"
			dump_cmd="mysqldump -h $myhost -P $myport -u $jdbc_user -p$jdbc_pass --databases $dbname | gzip -c"
		else
			dump_file="$checkpoint_dir/db-mysql.sql$COMPRESS_EXT"
			dump_cmd="mysqldump -h $myhost -P $myport -u $jdbc_user -p$jdbc_pass --databases $dbname | eval $COMPRESS_CMD"
		fi
		info "Dumping MySQL database: $dbname"
		eval "$dump_cmd" >"$dump_file"
		if [[ $? -ne 0 || ! -s "$dump_file" ]]; then
			rm -f "$dump_file"
			_die "Database dump failed (MySQL)."
		fi
		_db_meta_value="$(basename "$dump_file")"
	fi
fi
